...
def is_allowed_type(doc_tup: tuple,
                    doc_types: frozenset = frozenset(["start", "descriptor", "event", "stop"])
                    ) -> bool:
...
def basic_doc_stream(run: BlueskyRun) -> typing.Generator:
    ...
    for name, doc in run.canonical(fill="yes"):
        for name, doc in unpack_pages(name, doc):
            if name in ["start", "descriptor", "event", "stop"]:
                yield name, doc
