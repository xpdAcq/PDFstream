...
def render2(string: str, md: dict, formatter=pfmt):
    md = md.copy()
    md["time"] = timestampstr(md["time"])
    folder_tag_list = md.get("folder_tag_list", ["sample_name"])
    filename_prefix = get_filename_prefix(folder_tag_list, md)
    if re.fullmatch(r"/+", filename_prefix):
        filename_prefix = md.get("sample_name")
...
def render_and_clean(string, md, formatter=pfmt):
    """Render and clean the file name."""
    f = clean_template(render2(string, md, formatter=formatter))
